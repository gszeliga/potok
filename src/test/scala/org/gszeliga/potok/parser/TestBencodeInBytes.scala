...
class TestBencodeInBytes extends FlatSpec with Matchers {
  ...
  it must "parse a simple string" in {
    ...
    val result = BencodeInBytes.string(string).getOrElse(BString(Nil))
  ...
  it must "parse a space" in {
    ...
    val result = BencodeInBytes.string(str).getOrElse(BString(Nil))
  ...
  it must "parse a positive integer" in {
    ...
    val result = BencodeInBytes.int(int).getOrElse(BInt(0)).get
  ...
  it must "parse a list of strings" in {
    ...
    val result = BencodeInBytes.list(list).getOrElse(BList(Nil))
  ...
  it must "parse a single entry dictionary" in {
    ...
    val result = BencodeInBytes.dict(dict).getOrElse(BDict(Map.empty))
  ...
  it must "parse a whole file" in {
    val result = BencodeInBytes.parse(getClass().getResource("/samples/torrent/The.Walking.Dead.S04E01.HDTV.x264-ASAP.eztv.torrent"))
    //val result = BencodeInBytes.parse(getClass().getResource("/samples/torrent/enciclopedia-de-la-astronomia-y-el28.torrent"))
